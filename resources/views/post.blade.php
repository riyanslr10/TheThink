        ...
        <div class=" grid grid-cols-2 gap-4 justify-center  mx-[300px]   border-4 border-orange-200 rounded-lg shadow overflow-hidden "> 
            <a href="#"
                ...
                <div class="  leading-normal justify-center  dark:text-stone-600 ">
                    ...
                    <h1 class=" text-2xl font-bold tracking-tight text-stone-600"><a href="/posts?category={{ $post->category->slug }}" class="text-decoration-none"> {{ $post->category->name  }}</a></h1>
                    <p class=" text-xl font-semibold">By.<a href="/posts?author={{ $post->author->username }}" class="text-decoration-none">{{ $post->author->name }}</a></p>
                    <button type="button"
                        class="text-white text-center md:w-40 bg-orange-300 hover:bg-orange-400 focus:ring-4 focus:ring-amber-600font-medium rounded-lg text-lg font-bold px-3 py-2.5 me-2 my-2 dark:bg-amber-600 dark:hover:bg-amber-700 focus:outline-none dark:focus:ring-amber-700 
